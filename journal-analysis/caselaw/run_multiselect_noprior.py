#!/usr/bin/env python3
"""
LEAKAGE-FREE VARIANT (2026-07-25). Identical to run_multiselect_experiment.py
except that the rule "Most cases have 2-3 entailing paragraphs, not just one."
is deleted from both prompts and the remaining rules renumbered. That rule stated
the TEST answer-count distribution (mean 2.94) and contradicted the development
split it was nominally tuned on (mean 1.22).
...
OUTPUT_DIR = ROOT / "TASK2_code/runs_multiselect_noprior"
TOP_K = 20
...
4. If no paragraph genuinely entails the decision, return "none".
...
def call_openrouter(prompt, model, temperature=0.0, max_tokens=512, retries=8):
    """Retries hard, and records any call that never succeeded.

    The original returned the string "none" when the API failed, which is
    indistinguishable from the model legitimately selecting no paragraph. A
    run with silent failures scores like a run with genuine abstentions, so
    failures are recorded in FAILED_CALLS and reported before any score.
    """
    ...
    for attempt in range(retries):
        try:
            ...
            if attempt < retries - 1:
                ...
                FAILED_CALLS.append((model, str(e)))
                print(f"    GIVING UP on this case ({model})", file=sys.stderr)
                return "none"
...
def evaluate(preds, name):
    if FAILED_CALLS:
        print(f"  !! {len(FAILED_CALLS)} API calls failed permanently; "
              f"their cases scored as empty. THIS SCORE IS NOT VALID.")
    correct = retrieved = 0
...
FINAL = str(ROOT / "TASK2_code/FINAL_SUBMISSION/")
