...
The [`journal-analysis/`](journal-analysis/) folder holds the analysis code and
result files behind the two journal articles extending the proceedings paper
(Review of Socionetwork Strategies, COLIEE 2026 special issue): one on the
statute-law tasks, one on the case-law tasks. Each reported number traces to a
result file there, and the scripts that regenerate them verify the published
figures they depend on before writing anything new. The folder's README maps
each article to its analyses.
